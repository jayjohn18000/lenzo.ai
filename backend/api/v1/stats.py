# backend/api/v1/stats.py - New file for statistics endpoints
...
async def get_usage_statistics(days: int = Query(default=7, ge=1, le=365)) -> Dict[str, Any]:
    ...
    with get_db() as db:
        try:
            ...
            stats = db.query(
                ...
                DBQueryRequest.created_at >= start_date
            ).first()
            ...
            daily_usage_query = db.query(
                ...
                func.date(DBQueryRequest.created_at)
            ).order_by('date')
            ...
            model_stats = db.query(
                ...
                DBModelResponse.created_at >= start_date
            ).group_by(
                DBModelResponse.model_name
            ).order_by(
                func.count(DBModelResponse.id).desc()
            ).all()
            
            # Calculate model usage percentages
            total_model_calls = sum(row.usage_count for row in model_stats) if model_stats else 0
            
            top_models = []
            for row in model_stats[:4]:  # Top 4 models
                if total_model_calls > 0:
                    usage_percentage = (row.usage_count / total_model_calls) * 100
                    win_rate = (row.wins / row.usage_count) * 100 if row.usage_count > 0 else 0
            ...
            if len(model_stats) > 4:
                ...
                if others_count > 0 and total_model_calls > 0:
                    ...
                    others_avg_confidence = sum(row.avg_confidence * row.usage_count 
                                              for row in model_stats[4:]) / others_count
            ...
            return {
                ...
                "avg_response_time": round(float(stats.avg_response_time or 0) / 1000, 1),  # Convert to seconds
    ...
    with get_db() as db:
        try:
            ...
            today_stats = db.query(
                ...
                func.date(DBQueryRequest.created_at) == today
            ).first()
...
@stats_router.get("/models/performance")
async def get_model_performance(days: int = Query(default=7, ge=1, le=365)) -> Dict[str, Any]:
    ...
    with get_db() as db:
        try:
            ...
            model_metrics = db.query(
                ...
                func.avg(DBModelResponse.hallucination_risk).label('avg_hallucination_risk')
            ).filter(
                DBModelResponse.created_at >= start_date
            ).group_by(
                DBModelResponse.model_name
            ...
            for row in model_metrics:
                ...
                hallucination_risk = min(1.0, max(0.0, float(row.avg_hallucination_risk or 0)))
                
                performance_data.append({
                    "model": row.model_name,
                    "total_calls": row.total_calls,
                    "wins": row.wins or 0,
                    "win_rate": round((row.wins / row.total_calls * 100) if row.total_calls > 0 else 0, 1),
