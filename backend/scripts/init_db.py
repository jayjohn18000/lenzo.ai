    ...
    for days_ago in range(7, -1, -1):
        ...
        for _ in range(num_queries):
            ...
            for i, model in enumerate(selected_models):
                ...
                if is_winner:
                    confidence = min(
                        1.0, base_confidence + 0.05
                    )  # Winner boost, but bounded
                else:
...
def verify_data(session):
    ...
    over_confidence = (
        ...
        .filter(ModelResponse.confidence_score > 1.0)
        .count()
    )
    ...
    negative_confidence = (
        session.query(ModelResponse)
        .filter(ModelResponse.confidence_score < 0.0)
        .count()
    )
