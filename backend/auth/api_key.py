...
PRICING_TIERS = {
    ...
    "professional": {
        ...
        "features": ["advanced_routing", "multi_model", "analytics", "custom_models", "priority_support"]
    },
    "enterprise": {
        ...
        "features": ["all_features", "white_label", "dedicated_support", "custom_deployment"]
    }
}
...
class APIKeyManager:
    ...
    async def generate_api_key(self, user_id: int, name: str = "Default Key") -> Dict[str, Any]:
    ...
    async def check_rate_limits(self, user_id: int, api_key_id: int) -> bool:
        ...
        if day_count and int(day_count) >= api_key.requests_per_day:
            raise HTTPException(status_code=429, detail="Rate limit exceeded: requests per day")
    ...
    async def track_usage(self, 
                         ...
                         confidence_score: float) -> UsageRecord:
        """Track usage for billing and analytics"""
        
        # Calculate estimated cost
        estimated_cost = self._calculate_cost(user_id, total_tokens, len(models_attempted))
    ...
    async def get_usage_stats(self, user_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        avg_response_time = sum(r.response_time_ms for r in usage_records) / total_requests
        ...
        for record in usage_records:
            if record.winner_model:
                model_counts[record.winner_model] = model_counts.get(record.winner_model, 0) + 1
