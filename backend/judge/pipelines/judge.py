...
async def run_judge(req: RouteRequest, trace_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        citation_extractor = (
            asyncio.create_task(asyncio.to_thread(lambda: compile_citation_patterns()))
            if req.options.require_citations
            else None
        )
        ...
        winner, scores_by_trait, confidence = rank_and_select(
            candidates_with_heuristics, judge_scores, (winner_idx, consensus_score)
        )
...
def _log_performance_metrics(
    candidates: List[Candidate], total_time_ms: int, trace_id: str
):
...
def _can_use_fast_path(req: RouteRequest) -> bool:
    ...
    return (
        len(req.prompt) < 100
        and not req.expected_traits
        and req.options.model_selection_mode == "speed"
        and not req.options.require_citations
    )
