...
class EnhancedScorer:
    ...
    def calculate_trust_metrics_validated(
        self, candidate, judge_scores, all_candidates
    ):
        ...
        return TrustMetrics(
            reliability_score=ScoringValidator.validate_confidence(
                metrics.reliability_score
            ),
            consistency_score=ScoringValidator.validate_confidence(
                metrics.consistency_score
            ),
            hallucination_risk=ScoringValidator.validate_confidence(
                metrics.hallucination_risk
            ),
            citation_quality=ScoringValidator.validate_confidence(
                metrics.citation_quality
            ),
            confidence_score=ScoringValidator.validate_confidence(
    ...
    def _assess_hallucination_risk(self, text: str) -> float:
        ...
        # Risk calculation
        risk_score = min(
            1.0, (normalized_hallucination * 0.7) + (normalized_authority * 0.3)
        )
    ...
    def _calculate_consistency(
        self, target_candidate: Candidate, all_candidates: List[Candidate]
    ) -> float:
        """
    ...
    def rank_candidates_by_trust(
        self, candidates: List[Candidate], judge_scores: Dict[int, Dict[str, float]]
    ) -> List[Tuple[int, TrustMetrics]]:
        """
    ...
    def explain_selection(
        self, winner_metrics: TrustMetrics, winner_candidate: Candidate
    ) -> str:
        ...
        if reliability >= 0.9:
            explanation_parts.append(
                f"High-reliability model ({winner_candidate.model})"
            )
...
async def enhanced_consensus_selection(
    candidates: List[Candidate], judge_scores: Dict[int, Dict[str, float]]
) -> Tuple[Candidate, Dict, float, str]:
    """
