...
async def judge_candidates(
    candidates: List[Candidate], req: RouteRequest, trace_id: str
...
async def _batch_judge_candidates(
    ...
    try:
        ...
        return await _fallback_sequential_judge(
            candidates, rubric, judge_model, trace_id
        )
...
def _parse_batch_scores(
    response: str, num_candidates: int, rubric: Dict[str, float]
) -> Dict[int, Dict[str, float]]:
...
async def _judge_single_candidate(
    response_text: str, rubric: Dict[str, float], judge_model: str, trace_id: str
) -> Dict[str, float]:
    """
