...
class EnhancedConsensus:
    ...
    def _weight_trait_scores(
        self, trait_scores: Dict[str, float], rubric: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
    ...
    def _calculate_confidence(
        self, weighted_scores: Dict[int, float], score_details: Dict[int, Dict]
    ) -> float:
    ...
    def _calculate_margin(
        self, weighted_scores: Dict[int, float], winner_idx: int
    ) -> float:
    ...
    def _log_analysis(
        ...
        sorted_details = sorted(
            score_details.items(), key=lambda x: x[1]["weighted_score"], reverse=True
        )
