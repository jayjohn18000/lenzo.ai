...
def inject_citations(
    text: str, citations: List[Dict[str, Any]], format_style: str = "numbered"
    ...
    if format_style == "numbered":
        ...
        for i, citation in enumerate(citations, 1):
            ...
            if citation.get("url"):
                footnote_text += (
                    f"{i}. {citation.get('title', 'Reference')}: {citation['url']}\n"
                )
...
def _extract_context_around_citation(
    text: str, citation: str, context_length: int = 100
) -> str:
...
def format_citation_list(
    citations: List[Dict[str, Any]], style: str = "numbered"
) -> str:
    """
