...
class DataValidationMiddleware(BaseHTTPMiddleware):
    ...
    def validate_response_data(self, data: Any, parent_key: str = "") -> Any:
        ...
        if isinstance(data, dict):
            ...
            return [self.validate_response_data(item, parent_key=parent_key) for item in data]
            ...
            if isinstance(data, (int, float)):
                ...
                confidence_keywords = [
                    'confidence', 'score', 'percentage', 'rate', 
                    'probability', 'reliability', 'accuracy', 'certainty'
                ]
                
                if any(keyword in parent_key.lower() for keyword in confidence_keywords):
                    # Handle float values
                    ...
                    if float_value > 1.0 and float_value <= 100.0:
                        ...
                        if any(indicator in parent_key.lower() for indicator in decimal_indicators):
                            # Convert percentage to decimal
                            logger.debug(f"Converting {parent_key}={float_value} from percentage to decimal")
                    ...
                    # Cap values that exceed bounds
                        ...
                        logger.warning(f"Invalid {parent_key} value {float_value} capped at 1.0")
                        return 1.0
                    
                    # Handle negative values
                    elif float_value < 0.0:
                        logger.warning(f"Negative {parent_key} value {float_value} set to 0.0")
                        return 0.0
...
class ScoringValidator:
    ...
    def validate_confidence(confidence: float, source: str = "") -> float:
        ...
        if confidence < 0:
            ...
            if confidence <= 100:
                logger.info(f"Converting confidence {confidence} from percentage to decimal from {source}")
                return confidence / 100.0
            else:
                logger.warning(f"Confidence {confidence} exceeds valid range from {source}, capping at 1.0")
                return 1.0
        return confidence
    ...
    def validate_trait_scores(trait_scores: Dict[str, float]) -> Dict[str, float]:
        ...
        for trait, score in trait_scores.items():
            validated[trait] = ScoringValidator.validate_confidence(score, f"trait:{trait}")
        return validated
    ...
    def validate_response_scores(response_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "scores_by_trait" in response_data and isinstance(response_data["scores_by_trait"], dict):
            response_data["scores_by_trait"] = ScoringValidator.validate_trait_scores(
                response_data["scores_by_trait"]
            )
        
        if "model_scores" in response_data and isinstance(response_data["model_scores"], dict):
