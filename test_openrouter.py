    ...
    async def test_model(self, model: str, category: str = "") -> Dict:
        ...
        body = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Respond with exactly: 'Test successful for [model_name]'",
                }
            ],
            "max_tokens": 30,
        }
        ...
        try:
            async with httpx.AsyncClient(timeout=45.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   ✅ Success ({result['response_time']}s) - {usage.get('total_tokens', '?')} tokens"
                    )
                    ...
                    print(
                        f"   ❌ Failed ({result['response_time']}s): {error_msg[:80]}..."
                    )
    ...
    def print_summary(self):
        ...
        if successful:
            for result in successful:
                speed = (
                    ...
                    else "⚡" if result["response_time"] < 10 else "🐌"
                )
                tokens = (
                    f"({result['tokens_used']} tokens)" if result["tokens_used"] else ""
                )
                print(
                    f"   {speed} {result['model']} - {result['response_time']}s {tokens}"
                )
        else:
            print("   None")
        ...
        if successful:
            ...
            reliable_models = [
                r
                for r in successful
                if "openai" in r["model"] or "anthropic" in r["model"]
            ]
