// frontend/lib/api/unified-client.ts - UNIFIED API CLIENT
import { 
  QueryRequest, 
  QueryResponse, 
  UsageStats, 
...
export class UnifiedAPIClient {
  ...
  async query(request: QueryRequest, options?: { fast?: boolean }): Promise<QueryResponse> {
    ...
    console.log('🚀 Unified API Client Query:', {
      url,
  ...
  async getTodayStats(): Promise<{
    ...
    try {
      return await this.safeRequest('/dev/usage/today');
    } catch (error) {
      console.error('Today stats failed:', error);
  ...
  async getModelPerformance(): Promise<{
    ...
    try {
      return await this.safeRequest('/dev/models/performance');
    } catch (error) {
      console.error('Model performance failed:', error);
      // Return default values on error
      return {
        top_models: [],
        period_days: 7,
  ...
  async getJobStats(): Promise<{
    ...
    try {
      return await this.safeRequest('/dev/jobs/stats');
    } catch (error) {
